        ...
        <div className="grid lg:grid-cols-[55%_1fr] gap-10 lg:gap-14">
          ...
          <div className="space-y-4">
            <motion.div 
              ...
              className="relative aspect-[4/3] rounded-[2.5rem] overflow-hidden bg-white border border-gray-100 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] group cursor-grab active:cursor-grabbing"
            >
              ...
              <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 px-6 flex justify-between z-20 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button 
                  ...
                  className="w-12 h-12 rounded-full bg-white/90 backdrop-blur shadow-xl flex items-center justify-center text-heading hover:bg-primary hover:text-white transition-all pointer-events-auto active:scale-95"
                >
                  <ChevronLeft size={24} />
                </button>
                <button 
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2.5 z-20 p-2 rounded-full bg-black/10 backdrop-blur-md border border-white/10 group-hover:scale-110 transition-transform">
                {product.images.map((_, idx) => (
                  <button
                    ...
                    className={`h-2 transition-all rounded-full ${activeImage === idx ? 'w-8 bg-primary shadow-[0_0_12px_rgba(245,158,11,0.5)]' : 'w-2 bg-white/40 hover:bg-white'}`}
                  />
                ))}
              </div>
            ...
            <div className="grid grid-cols-4 gap-3 px-1">
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`aspect-square rounded-[1.25rem] overflow-hidden border-2 transition-all duration-300 relative group ${
                    activeImage === idx 
                      ? 'border-primary shadow-[0_0_15px_rgba(245,158,11,0.3)] scale-105 z-10 bg-white' 
                      : 'border-white bg-gray-50 opacity-100 hover:border-primary/30 hover:shadow-lg'
                  }`}
                >
                  ...
                  {activeImage !== idx && <div className="absolute inset-0 bg-heading/5 group-hover:opacity-0 transition-opacity" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            >
              <div className="flex items-center gap-3 mb-5">
                <span className="bg-primary text-white px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] border border-primary/20 shadow-sm">
                  {product.category}
                </span>
                <span className="text-white/80 font-bold uppercase tracking-[0.12em] text-[10px] flex items-center gap-2 bg-gray-800 border border-gray-700 px-3.5 py-1.5 rounded-xl shadow-sm">
                  ...
                  Ref: <span className="text-white font-black">{product.refNumber}</span>
                </span>
              </div>

              <h1 className="text-3xl lg:text-4xl font-display font-black text-heading mb-3 leading-[1.1] tracking-tight">
                {product.name}
              </h1>

              <div className="bg-gray-50/60 rounded-2xl p-4 mb-5 border border-gray-100 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-2 opacity-[0.04] rotate-12 pointer-events-none">
                ...
                <div className="flex items-center justify-between gap-4 relative z-10">
                  <div>
                    ...
                    <h2 className={`text-4xl font-display tracking-tighter drop-shadow-sm whitespace-nowrap transition-all duration-300 ${
                      isSold ? "text-[#9CA3AF] line-through font-medium" : "text-primary font-black"
                    }`}>
              ...
              <div className="grid grid-cols-2 gap-2.5 mb-5">
                {specifications.map((spec, i) => (
                  <div key={i} className="flex flex-col gap-1 bg-white border border-gray-100 rounded-xl px-3.5 py-3 shadow-sm hover:border-primary/20 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <button
                  ...
                  disabled={isSold}
                  className={`w-full py-4 px-8 flex items-center justify-center gap-3 rounded-2xl font-black uppercase tracking-widest text-[13px] transition-all duration-300 group ${
                    isSold 
                      ? "bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200" 
                      : "bg-primary text-white shadow-[0_16px_32px_-8px_rgba(245,158,11,0.45)] hover:shadow-[0_20px_40px_-8px_rgba(245,158,11,0.6)] hover:-translate-y-1 hover:scale-[1.02]"
                  }`}
                >
              ...
              {/* WhatsApp CTA */}
              <div
                onClick={() => !isSold && window.open("https://wa.me/918778868739", "_blank")}
                className={`mt-3 w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl transition-all duration-300 font-black uppercase tracking-widest text-[12px] group ${
                  isSold
                    ? "bg-gray-100/50 text-gray-400 cursor-not-allowed border border-gray-100 opacity-70"
                    : "bg-[#25D366]/10 border border-[#25D366]/25 text-[#1a9e4d] hover:bg-[#25D366] hover:text-white hover:border-[#25D366] hover:shadow-[0_8px_20px_-6px_rgba(37,211,102,0.5)] hover:-translate-y-0.5"
                }`}
              >
              ...
              <div className="mt-5 flex items-center gap-4 p-3.5 rounded-2xl bg-orange-50/70 border border-orange-100">
                ...
                <p className="text-[12px] text-heading/70 font-bold leading-tight">
                  🔥 <strong className="text-heading">18+ buyers</strong> currently interested in <strong className="text-primary">{product.category}</strong>
                </p>
              </div>
            </motion.div>
          </div>
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-5">
          {[
            ...
            <motion.div 
              ...
              className="p-8 rounded-[2rem] border border-gray-100 bg-white hover:border-primary/20 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group cursor-default"
            >
              <div className="w-14 h-14 rounded-2xl bg-primary/8 flex items-center justify-center mb-5 group-hover:bg-primary/15 transition-colors shadow-sm">
